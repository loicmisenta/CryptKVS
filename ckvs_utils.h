...
#define ARG_SIZE_STATS 0
#define ARG_SIZE_GET 2
#define ARG_SIZE_SET 3
#define ARG_SIZE_NEW 2
...
/**
 * Struct containing the key and the corresponding value of a json_object
 */
typedef struct{
    const char* key;
    json_object* value;
...
/**
 * @brief Print the Header and the Keys that are contained in a json formatted string (used for command stats)
 *
 * @param in (const char*) pointer to the char* containing the json response
 * @return int, error code
 */
int print_json_response_stats(const char* json);

/**
 * @brief Compare the number of given arguments to an expected size
 *
 * @param int optargc number of arguments to verify
 * @param int expected_size to which optargc should be compared
 * @return int, error code (ERR_NOT_ENOUGH_ARGUMENTS or ERR_TOO_MANY_ARGUMENTS
 */
int check_arg_size(int optargc, int expected_size);

/**
 * @brief free the memory of the two given pointers
 *
 * @param first pointer to free
 * @param second pointer to free
 */
